...
void partitionDecreasingHelper(int budget, int *partition, int pos)
{
  ...
  for (spending = budget; spending > 0; spending--) {
    ...
    if (pos == 0 ) partitionDecreasingHelper(budget-spending, partition, pos+1);
  }
}
...
void partitionOddAndEvenHelper(int budget, int *partition, int pos)
{
  ...
  for (spending = 1; spending <= budget; spending++) {
    partition[pos] = spending;
    if (pos == 0) partitionOddAndEvenHelper(budget - spending, partition, pos+1);
    if (pos != 0) {
      if (partition[pos - 1] % 2 == 0) {
	if (partition[pos] % 2 != 0) {
